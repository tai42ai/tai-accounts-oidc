...
All config here is the plugin's OWN env-based settings (the ``TAI_`` family
prefix, matching the other accounts member). It is entirely distinct from the
skeleton-INJECTED ``AccountsProviderSettings`` object the provider factory
receives (its ``.redis`` reach) — that injected object is stashed in the
``provider.py`` module holder, never read from here.

``providers`` is parsed from the ``TAI_ACCOUNTS_OIDC_PROVIDERS`` env var as JSON
(pydantic-settings' native complex-field parsing). ``state_key`` and
``public_base_url`` are REQUIRED, but carry a ``None`` sentinel default so a
missing value surfaces as a precise ``ValueError`` naming the env var at provider
construction (``provider.py``) rather than an opaque settings-load failure.
"""
...
# The default OIDC scope set. A module constant so the mutable list is never a
# shared class-attribute default (RUF012); each config gets its own copy.
...
class ProviderDisplay(BaseModel):
    ...
    ``label`` may be left empty in config — a preset fills it (e.g. "Continue
    with Google"). ``icon`` is an optional operator-supplied inline SVG string
    (``currentColor`` convention); no brand marks ship in this repo.
    """
...
def _normalize_public_base_url(value: str) -> str:
    ...
    strip a trailing slash so the derived ``redirect_uri`` / hand-back never
    doubles a separator."""
...
class OidcAccountsSettings(TaiBaseSettings):
    ...
    # HMAC key for the tamper-evident ``state`` parameter and REQUIRED. The None
    # sentinel lets the provider raise a precise ValueError naming the env var.
    state_key: SecretStr | None = None

    # The public origin BOTH the OAuth redirect_uri and the /login?sso= hand-back
    # derive from — never the inbound request's Host/origin. REQUIRED (None
    # sentinel → precise ValueError at provider construction).
    ...
    def _validate_providers(self) -> OidcAccountsSettings:
        # Duplicate names would make provider lookup by name ambiguous; reject at
