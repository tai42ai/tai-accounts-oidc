...
async def test_authorize_binding_cookie_secure_on_uppercase_https(make_provider: Any, oidc_server: Any) -> None:
    oidc_server.configure_oidc()
    # An uppercase scheme validates as https (schemes are case-insensitive), so the
    # cookie must still be Secure — the flag is derived from the parsed scheme, not a
...
async def test_authorize_binding_cookie_not_secure_on_loopback_http(make_provider: Any, oidc_server: Any) -> None:
    oidc_server.configure_oidc()
    # A loopback-http origin (the only non-https form the settings accept) drops the
    # Secure flag so the cookie round-trips for non-browser clients on that origin;
    # the binding still holds (HttpOnly + SameSite + value match are unaffected).
...
async def test_callback_missing_binding_cookie_rejected(
    ...
    # A genuine state redeemed WITHOUT the flow cookie — a state minted in a
    # different browser (login-CSRF / session fixation) — is refused before any
    # token exchange.
    with caplog.at_level(logging.WARNING):
