...
- ``FakeRedis`` + ``make_client_ctx`` — the string/TTL operations the state,
  SSO-code, and session records use (``set(ex=)`` / ``get`` / ``getdel`` /
  ``delete`` / ``pexpire``), yielded through the ``client_ctx`` seam the provider
  imports. No real Redis.
- A real loopback OIDC issuer (``oidc_server``) serving discovery/JWKS and a
  configurable token/userinfo endpoint, minting real RSA-signed id_tokens — so the
  login flow's httpx token exchange and the kit id_token verification run end to
  end with zero external network I/O.
...
class FakeRedis:
    ...
    ``ex`` on ``set`` and ``pexpire`` are recorded so a test can assert the TTL
    posture (idle-slide, short SSO window) without a real clock. ``getdel`` is the
    single-use read the state/SSO records depend on.
    """
...
def build_request(
    ...
    *,
    ...
    """A Starlette ``Request`` carrying an optional JSON body, query string, and
    path params — enough to drive a route handler directly."""
...
def make_provider(monkeypatch: pytest.MonkeyPatch):
    ...
    fake_redis)``. ``state_key``/``public_base_url`` default to valid values; pass
