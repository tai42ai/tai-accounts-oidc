# Contributing to tai-accounts-oidc

`tai-accounts-oidc` is the OIDC/OAuth2 login **accounts provider** for the TAI
...
- `src/tai_accounts_oidc/provider.py` — the `OidcAccountsProvider`
  (session-token validation, Redis state/SSO-code/session storage) and its
  registration.
- `src/tai_accounts_oidc/routes.py` — the public `/api/login/oidc/*` authorize +
  callback routes and `/api/login/sso/exchange`.
- `src/tai_accounts_oidc/oauth.py` — state signing/verification, PKCE, and the
  flow's random tokens.
- `src/tai_accounts_oidc/presets.py` — the issuer presets (`auth0`, `google`,
  `okta`, `keycloak`, `azure`, `github`) and provider-config resolution.
- `src/tai_accounts_oidc/settings.py` — the `TAI_ACCOUNTS_OIDC_*` settings and
