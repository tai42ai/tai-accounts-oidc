...
``provider.provider_settings()`` (the module holder populated at provider init).
This module also registers the AC-required boot guard.

Success bodies are the frozen ``{"data": {"token": raw, "user_id": ...}}`` shape.
Every failure branch returns a generic envelope with a 4xx/5xx status and writes
the detailed cause to the server log — never a silent redirect-to-login.
"""
...
# The flow-binding cookie: an HttpOnly secret set at authorize and required to
# match the redeemed state's binding at callback, so a state minted in one browser
...
def _state_key() -> bytes:
    # REQUIRED and validated at provider construction, so it is never None by the
    # time a route runs; assert for the type checker and fail loud if that breaks.
...
def _redirect_uri(name: str) -> str:
    # Derived from operator config, NEVER the inbound request's Host/origin: a
    # Host-header-injecting client must not be able to steer where the auth code
    # lands.
    return _public_base_url() + _CALLBACK_PATH_TEMPLATE.format(name=name)


def _flow_cookie_secure() -> bool:
    # Match the Secure flag to the scheme the deployment is served on: an https
    # origin gets a Secure cookie; a loopback-http origin (the only non-https form
    # the settings accept) does not, so the cookie round-trips for every client on
    # that origin instead of only browsers that treat loopback as a secure context.
    # Parse the scheme (case-insensitive, like the settings validator) so an
    # uppercase ``HTTPS`` origin is not mistaken for non-https and left unprotected.
...
async def oidc_authorize(request: Request) -> Response:
    ...
    # The OIDC id_token replay binding; GitHub is plain OAuth2 with no id_token to
    # bind, so it carries no nonce.
    if config.kind == "oidc":
        params["nonce"] = nonce
...
async def oidc_callback(request: Request) -> Response:
    ...
    operator provisions its policy (deny-by-default). Every failure branch returns
    a generic envelope with the detailed cause written to the server log first.
    """
    name = request.path_params["provider"]
    runtime = provider.get_runtime(name)
    if runtime is None:
    ...
    if "error" in query:
        # The user cancelled at consent, or the issuer refused — log the detail,
        # never a bare redirect to /login with no context.
        logger.warning(
    ...
    # A state minted for provider A must never be redeemed at provider B's
    # callback — verify BEFORE any token exchange.
    ...
    # Bind the redemption to the browser that began the flow: the state's binding
    # must match the flow cookie set at authorize. A missing/mismatched cookie is a
    # state minted in another browser (login-CSRF / session fixation) — reject
...
def _assert_accounts_provider_instantiated() -> None:
    ...
    injected and the holder is empty. Registered from the route layer so it exists
    EXACTLY when the login HTTP surface is mounted; the boot probe (when AC is
    enabled) populates the holder before this runs, so this fires only in the
